...
public:
    ...
    __host__ __device__
        BoundingBox(float3 bMin, float3 bMax) : bmin(bMin), bmax(bMax), bEmpty(false), dirty(true) {}

    __host__ __device__ __inline__
        float3 getMin() const { return bmin; }
    __host__ __device__ __inline__
        float3 getMax() const { return bmax; }
    __host__ __device__
        bool isEmpty() { return bEmpty; }

    __host__ __device__
        void setMin(float3 bMin) {
        bmin = bMin;
        dirty = true;
        bEmpty = false;
    }
    __host__ __device__
        void setMax(float3 bMax) {
        bmax = bMax;
        dirty = true;
        bEmpty = false;
    }
    ...
    __host__ __device__
        ...
        float3 R0 = make_float3(r.origin[0], r.origin[1], r.origin[2]);
        float3 Rd = make_float3(r.direction[0], r.direction[1], r.direction[2]);
